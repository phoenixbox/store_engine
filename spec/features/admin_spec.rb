...
describe "admin dashboard" do
  context "when an admin visits their dashboard" do
    xit "should have a list of all orders" do
      user = FactoryGirl.create(:user)
      order1 = FactoryGirl.create(:order, user: user)
      product1 = FactoryGirl.create(:product)
      order2 = FactoryGirl.create(:order, user: user)
      # product2 = FactoryGirl.create(:product, title: "Banana")
      order1.order_items << FactoryGirl.create(:order_item, product: product1)
      order2.order_items << FactoryGirl.create(:order_item, product: product1)
      visit '/admin/dashboard'

      # expect(page).to have_link(admin_order_path(order1))
      # expect(page).to have_link(admin_order_path(order2))
